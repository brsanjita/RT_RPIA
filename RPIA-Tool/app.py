...
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:password@localhost/rpiadb'
...
class AssetDamage(db.Model):
    ...
    a_probability_of_impact = db.Column(db.String(50))
    a_impact_index = db.Column(db.String(50))
   

    def __init__(self, a_type, a_id, a_location, a_specifications, a_criticality_index, a_probability_of_impact, a_impact_index):
        self.a_type=a_type
        self.a_id=a_id
        self.a_location=a_location
        self.a_specifications=a_specifications
        self.a_criticality_index=a_criticality_index
        self.a_probability_of_impact=a_probability_of_impact
        self.a_impact_index=a_impact_index
...
class ThreatInfoSchema(ma.Schema):
    class Meta:
        fields = ('key_code','key_desc','key_val')
threat_info_schema = ThreatInfoSchema()
threat_infos_schema = ThreatInfoSchema(many=True)  
...
class AssetDamageSchema(ma.Schema):
    class Meta:
        fields = ('a_type', 'a_id','a_location', 'a_specifications', 'a_criticality_index','a_probability_of_impact','a_impact_index')
...
@app.route('/threatinfo', methods = ['GET', 'POST'])
def threat_info():
    if request.method == 'POST':
        f = request.files['file']
        fstring = f.read()
        ...
        for k in csv_dicts:
            try:
                updateStatus = ThreatInfo.query.filter_by(key_code=k).update(dict(key_val=csv_dicts[k]))
                if updateStatus == 0: 
                    return 'Unsuccessful. Please check the file structure'
                db.session.commit()
            except:
    ...
    if request.method == 'GET':  
        query_result = ThreatInfo.query.all()
        final_result = threat_infos_schema.dump(query_result) 
        return jsonify({'data': final_result})
...
def asset_damage():
    if request.method == 'POST':
        ...
        for i in range(len(data)): 
            assetRow = AssetDamage(str(data['a_type'][i]), str(data['a_id'][i]), str(data['a_location'][i]), str(data['a_specifications'][i]), str(data['a_criticality_index'][i]), str(data['a_probability_of_impact'][i]), str(data['a_impact_index'][i]))
            try:
